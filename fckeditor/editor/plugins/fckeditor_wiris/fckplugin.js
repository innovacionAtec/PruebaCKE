...
parent._wrs_conf_getlatexPath = FCKConfig.PluginsPath + 'fckeditor_wiris/integration/getlatex.php';				// Specifies where is getlatex script
...
function wrs_int_openNewFormulaEditor(iframe) {
	...
	if (parent._wrs_conf_usePopUps) {
		if (parent._wrs_int_window_opened) {
			...
			parent._wrs_int_window = parent.wrs_openEditorWindow(null, iframe, true);
		}
	}
	else {
...
function wrs_int_openNewCAS(iframe) {
	...
	if (parent._wrs_conf_usePopUps) {
		if (parent._wrs_int_window_opened) {
			...
			parent._wrs_int_window_opened = true;
			parent._wrs_int_window = parent.wrs_openCASWindow(iframe, true);
		}
	}
	else {
 ...
 * @param string mathml
 */
parent.wrs_int_updateFormula = function (mathml, editMode) {								// We need instance this function on parent object because core/editor.js only can access to the parent object.
	parent.wrs_updateFormula(parent._wrs_int_temporalIframe.contentWindow, parent._wrs_int_temporalIframe.contentWindow, mathml, parent._wrs_int_wirisProperties, editMode);
}
